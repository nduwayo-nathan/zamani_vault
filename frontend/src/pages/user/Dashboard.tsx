...
const UserDashboard = () => {
  ...
  const getFreeContent = () => popularContent.filter(content => !content.isPremium);
  const getPremiumContent = () => popularContent.filter(content => content.isPremium);
              ...
              <div className="relative aspect-video">
                ...
                {content.isPremium && (
                  <div className="absolute top-2 left-2">
                    <Badge variant="secondary" className="bg-primary border-0 text-[10px]">
                      Premium
                    </Badge>
                  </div>
                )}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          {getPremiumContent().map((content) => (
            <Card 
              key={content.id} 
              className={`overflow-hidden hover-card-scale ${user?.subscription !== 'free' ? 'bg-card/50' : 'bg-card/20 opacity-70'}`}
              onClick={() => user?.subscription !== 'free' ? handleShowDetails(content.id) : navigate('/user/subscription')}
            >
              <div className="relative aspect-video">
                <img
                  src={content.image}
                ...
                {user?.subscription === 'free' && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60">
                    <Badge variant="outline" className="border-yellow-500 text-yellow-500 mb-2">Premium</Badge>
                    <span className="text-xs text-center px-4">Upgrade to view</span>
                  </div>
                )}
                {user?.subscription !== 'free' && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 hover:opacity-100 transition-opacity">
